...
| `payment-service` | 下单/回调/对账：`application` 管事务，`infrastructure` 含 Redis 回调短锁与**对账调度**；**`POST /internal/payments/orders/retry-credit`** 在 `presentation`（仅运维/服务间，验 `X-Internal-Token`） | `common-web` |
