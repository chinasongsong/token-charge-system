...
flowchart TD
  ...
  C --> D[IdempotencyGatewayFilter<br/>+12]
  D --> E[IpRiskAndQuotaGatewayFilter<br/>+13]
  E --> F[ApiKeyRedisRateLimitGatewayFilter<br/>+14]
  F --> G[BillingPreflightGatewayFilter<br/>+15]
  G --> H[Spring 内置过滤器 + 路由匹配]
  H --> I[下游服务]
```
...
| +12 | `IdempotencyGatewayFilter` | Chat 幂等复合键（O-10，`X-Idempotency-Key`） |
| +13 | `IpRiskAndQuotaGatewayFilter` | IP 黑白名单、日配额（O-6，默认关） |
| +14 | `ApiKeyRedisRateLimitGatewayFilter` | 秒级 QPS（Redis 计数） |
...
3. **先解析 Key、再合成幂等键**：扣费幂等见 `IdempotencyGatewayFilter`；`X-Trace-Id` 主要用于观测。
4. **再风控/限流**：日配额与 QPS 依赖 `X-Api-Key-Id` / `X-User-Id`。
5. **预检放最后**：需要已解析的 `userId`，且仅针对最贵的 Chat 路径。
...
| [filters/08-IdempotencyGatewayFilter.md](./filters/08-IdempotencyGatewayFilter.md) | Chat 扣费幂等 O-10 |
